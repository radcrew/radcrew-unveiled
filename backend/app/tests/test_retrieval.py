...
from app.chatbot.graph.nodes.rag_answer.retrieval import retrieve_relevant_chunks
...
def test_semantic_results_used_when_above_threshold(mock_sem) -> None:
    ...
    out = retrieve_relevant_chunks(_corpus(), "who wrote the compiler", limit=2)
...
def test_lexical_fallback_rescues_name_question(mock_sem) -> None:
    ...
    out = retrieve_relevant_chunks(_corpus(), "what did Ada Lovelace build?", limit=2)
...
def test_returns_empty_when_nothing_matches(mock_sem) -> None:
    ...
    out = retrieve_relevant_chunks(_corpus(), "zzz qqq", limit=2)
    assert out == []
